...
import { useWishlist, type WishlistFolder, type WishlistProduct } from '@/components/wishlist-context';
...
export default function WishlistFolderPage() {
    ...
    void fetch(apiUrl(`/api/store/wishlists/${encodeURIComponent(folderId)}`), {
      ...
      .then(async (response) => {
        const body = (await response.json().catch(() => null)) as
          | { data?: { folder?: WishlistFolder }; error?: string; message?: string }
          | null;
        if (!response.ok || !body?.data?.folder) {
          throw new Error(body?.error ?? body?.message ?? 'Unable to load this wishlist.');
        }
        if (mounted) setFolder(body.data.folder);
      })
      .catch((cause) => {
        if (mounted) {
          setError(cause instanceof Error ? cause.message : 'Unable to load this wishlist.');
        }
      })
      .finally(() => {
  ...
  const openAction = (nextAction: Exclude<Action, null>, product: WishlistProduct) => {
  ...
  const mutateProduct = async () => {
    if (!folder || !activeProduct || !action) return;
    setRequesting(true);
    setError('');
    try {
      const path =
      ...
      const response = await fetch(apiUrl(path), {
        ...
        headers: {
          ...
          ...(action === 'move' ? { 'Content-Type': 'application/json' } : {}),
        },
        ...(action === 'move'
      ...
      const body = (await response.json().catch(() => null)) as
        ...
        | null;
      if (!response.ok) {
        throw new Error(body?.error ?? body?.message ?? 'Unable to update this wishlist.');
      }
      setFolder((current) =>
        current
          ? {
              ...
              productIds: current.productIds.filter((id) => id !== activeProduct.id),
              products: current.products.filter((product) => product.id !== activeProduct.id),
            }
          : current
      );
      await refresh();
      ...
      setError(cause instanceof Error ? cause.message : 'Unable to update this wishlist.');
    } finally {
      setRequesting(false);
    }
  };
  ...
  return (
    <div className="min-h-[calc(100svh-4rem)] bg-background">
            ...
            <header className="flex items-center justify-between gap-4 border-b border-border pb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h1 className="mt-1 text-2xl font-semibold tracking-tight">{folder.name}</h1>
                </div>
              </div>
              <span className="text-xs font-semibold text-muted-foreground">
                {folder.totalProducts} {folder.totalProducts === 1 ? 'item' : 'items'}
              </span>
            </header>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1 py-1 pr-8">
                      <Link href={`/product/${product.slug}`} className="group">
                        ...
                        {product.price !== null ? (
                          <p className="mt-3 text-sm font-semibold">{formatPriceCents(product.price)}</p>
                        ) : null}
                      </Link>
                    </div>
                    <Popover>
      ...
      <Dialog open={action === 'move'} onOpenChange={(open) => !open && closeAction()}>
        <DialogContent>
          <DialogHeader>
          ...
          {otherFolders.length > 0 ? (
            <Select value={targetFolderId} onValueChange={(value) => setTargetFolderId(value ?? '')}>
              ...
              <SelectContent>
                {otherFolders.map((item) => {
                  const FolderIcon = iconMap[item.icon as keyof typeof iconMap] ?? Heart;
                  return (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={closeAction} disabled={requesting}>Cancel</Button>
            <Button onClick={() => void mutateProduct()} disabled={requesting || !targetFolderId}>
              {requesting ? <Loader2 className="animate-spin" /> : 'Move product'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={action === 'delete'} onOpenChange={(open) => !open && closeAction()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Remove from wishlist?</DialogTitle>
            <DialogDescription>
              This will remove the product from <strong>{folder?.name}</strong>. You can save it again later.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={closeAction} disabled={requesting}>Cancel</Button>
            <Button variant="destructive" onClick={() => void mutateProduct()} disabled={requesting}>
              {requesting ? <Loader2 className="animate-spin" /> : 'Delete product'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
